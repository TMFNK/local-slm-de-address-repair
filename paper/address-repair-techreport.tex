...
seconds; base took 79.5 minutes for 2,000 records (p50 2237.1~ms,
p95 3969.4~ms); v3 SFT took 50.8 minutes (p50 1443.7~ms, p95
2293.3~ms).
Model-load time and peak memory were not measured; only per-record
latency is reported.
...
nothing. Only per-record latency was measured, on one 8~GB machine;
load time and peak memory are open. The frozen comparison is one
deterministic run per system. The validation-subsample scores that
...
\cite{repo} at evaluation commit \texttt{65c192c}; the v3 Colab
training run used commit \texttt{3464395}. Setup uses Python 3.12 with a \texttt{uv}
...
(\texttt{docs/REPRO.md}); \texttt{make\_report.py} remains a
placeholder and is not treated as a result. Each frozen run checks the
