...
\cite{repo}, released as immutable tag \texttt{v0.1.0}. The v3 frozen
result boundary is commit \texttt{c83165aab5c4249d986084a74122e273c7d42b75};
its Colab training run used commit \texttt{3464395}. The rejected GRPO
result boundary is commit \texttt{0ecbfe1e9a53b3547b297d971bdfa9330e35eaf8},
following the run code at \texttt{90e0887}; the server-tolerance harness
change only converts an abort into counted unusable rows and cannot move any
other number. The rejected v5p1 result boundary is commit
\texttt{7f28b03ad0641ad76e21be4b7458e07362e0eec9}; its evaluation metadata
records the run code boundary at \texttt{6c2350e}. The Zenodo software DOI
placeholder is \texttt{PLACEHOLDER\_ZENODO\_DOI}, with URL
\url{https://doi.org/PLACEHOLDER\_ZENODO\_DOI}; replace both after deposition
assigns the release record. Setup uses Python 3.12 with a \texttt{uv}
...
\bibitem{repo} MbitAI, ``local-slm-de-address-repair'' (Apache-2.0;
data ODbL-1.0).
